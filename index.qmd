...
```{=html} 
<!-- ==== RAISINS compact badge (with instructions, inner glow, crisp CTA) ==== -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons/font/bootstrap-icons.css" />
...
<div id="rb" role="note" aria-live="polite">
  ...
  <div class="rb-text">
    <span class="headline">
      <strong>₹200/month</strong> • <em>Free preview mode</em>
    </span>
...
#rb{
  ...
  max-width:90vw;
  font:600 .92rem/1.25 system-ui,-apple-system,"Segoe UI",Roboto,Arial;
  color:#102033;
  ...
  box-shadow:inset 0 0 6px rgba(255,183,3,0.3);
}
...
/* icon */
#rb .bi{ color:var(--amber); font-size:1.05rem; }

/* text block */
#rb .rb-text{ display:flex; flex-direction:column; gap:.15rem; }
#rb .headline em{ font-style:normal; color:var(--blue); }
#rb .instructions{
  font-weight:500; font-size:.85rem; color:#333; line-height:1.25;
}
#rb .instructions small{ color:#555; font-size:.78rem; }
...
/* close button */
#rb .rb-x{
  appearance:none; border:0; background:transparent; color:#555;
  font-size:1rem; line-height:1; cursor:pointer; padding:.1rem .2rem;
}
#rb .rb-x:hover{ color:#000; }

/* responsive */
@media (max-width:600px){
  #rb{ flex-direction:column; align-items:flex-start; bottom:72px; padding:.6rem .8rem; border-radius:1rem; }
  #rb .rb-go{ align-self:center; margin-top:.35rem; }
}
...
<script>
(function(){
  const b=document.getElementById('rb'), x=b.querySelector('.rb-x');
  if(sessionStorage.getItem('rb-hide')==='1') b.style.display='none';
  x.addEventListener('click',()=>{b.style.display='none';sessionStorage.setItem('rb-hide','1');});
})();
</script>
```  
